...
const PokeTable = () => {
  ...
  const displayimage = filteredPokemon.slice(
    currpage * pagelimit - pagelimit,
    currpage * pagelimit,
  );
    ...
    <div className="w-11/12 mx-auto">
      ...
      <table className="w-full cursor-pointer">
        ...
        <tr className="bg-amber-400">
          <th rowSpan={2} className="w-4 border-2">
            S.No.(#)
          </th>
          <th rowSpan={2} className="w-1/6 border-2">
            Pokemon Name{" "}
          </th>
          <th rowSpan={2} className="w-24 h-24 border-2">
            {" "}
            Image{" "}
          </th>
          <th colSpan={5}>Abiities</th>
        </tr>
          ...
          {isLoading
            ...
            : displayimage.map((item, index) => (
                <Pokerow
                  ...
                  key={index}
                  index={index}
                  offset={pagelimit * currpage - pagelimit}
                />
              ))}
        </tbody>
      </table>
      <div className="flex gap-6 w-full justify-center my-10">
        <button
          className={`border-2 py-2 px-4 bg-amber-400 hover:opacity-90 active:opacity-70 ${currpage == 1 ? "cursor-not-allowed" : ""}`}
        ...
        <button
          className={`border-2 py-2 px-4 bg-amber-400 hover:opacity-90 active:opacity-70 ${filteredPokemon.length < pagelimit * currpage ? "cursor-not-allowed" : ""}`}
          disabled={filteredPokemon.length < pagelimit * currpage}
